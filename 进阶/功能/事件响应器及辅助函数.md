...
`aliases` 是一个 `Set[Union[str, Tuple[str, ...]]]` 对象，用于存放*命令别名*，通常和 [`cmd`](#msgcmd) 同时出现。其中每一个元素的作用均等价于 [`cmd`](#msgcmd) 的作用。

值得注意的是，我们并不能仅使用 `aliases` 而不填写 [`cmd`](#msgcmd)，`aliases` 仅作为 [`cmd`](#msgcmd) 的补充而不是替代。
...
matcher_regex = matcher_group.on_regex(b'^regex$', priority=15)
```
...
matcher_regex = on_regex(b'^regex$', priority=15, block=False)
```
